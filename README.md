...
用于在个人服务器部署的模拟购物网站，专门为爬虫/自动化 Agent 的验证码通过能力测试而设计。站点提供常见的内置验证码类型，并在每次打开页面时强制弹出、随机轮换，以最大化覆盖不同处理逻辑。站点同时采集行为日志与可疑特征用于分析与改进。

## 功能特性
- 页面结构：首页、商品列表、商品详情、购物车、结算、登录、注册
- 强制验证码：每次打开新页面都会跳转到验证码页，通过后仅本次回跳放行
